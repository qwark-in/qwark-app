import Handlebars from "handlebars";

import { space, baseColor, semanticColor } from "./qwarkTokens";
import * as fs from "fs";
import * as path from "path";

const generate = () => {
  console.log("Generating Qwark design system files...");
  console.log("Reading template");
  // Note: Script to run this runs with packages/config as the PWD
  const template = fs.readFileSync(
    path.resolve(__dirname, "qwarkDesignSystemTemplate.txt")
  );
  const compiledTemplate = Handlebars.compile(template.toString());
  fs.writeFileSync(
    path.resolve(__dirname + "/qwarkDesignSystem.ts"),
    compiledTemplate({
      spaceTokens: Object.entries(space),
      baseColors: Object.entries(baseColor),
      semanticColors: Object.entries(semanticColor[0]),
      // semanticColorsDark: Object.entries(semanticColor[1]),
    })
  );
  console.log("Generation complete!");
};

generate();
