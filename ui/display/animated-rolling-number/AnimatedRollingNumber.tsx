...
export interface AnimatedRollingNumberProps
  ...
  /** Whether numbers are visible or hidden. Defaults to true */
  isVisible?: boolean;
...
export const AnimatedRollingNumber: React.FC<AnimatedRollingNumberProps> = ({
  ...
  const formattedNumber = useMemo(
    () =>
      ...
      numberValue.toLocaleString(locale, {
        ...
        currency: 'INR',
        style: 'currency',
        // TODO: Check what can be done with the and toFixed() of parent
  ...
  const number = useMemo(() => {
    ...
    const numberWithSign = ((showPlusSign && value >= 0 ? '+' : '') + formattedNumber).split('');
    if (I18nManager.isRTL) {
      return numberWithSign.reverse();
    }

    return numberWithSign;
  }, [formattedNumber, value, showPlusSign, isVisible]);
  ...
  return (
    <Animated.View layout={LinearTransition} style={[styles.container, containerStyle]} {...rest}>
