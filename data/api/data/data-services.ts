...
export const getDashboardData = async (authData: AuthDataType) => {
  try {
    const response = await axios.get<GetDashboardResponse>(`${BASE_URL}/dashboard`, {
      headers: {
    ...
    if (axios.isAxiosError(err)) {
      ...
      throw new Error('An unexpected error occurred while fetching the dashboard data.');
    }
  }
};
...
export const getMarketData = async (authData: AuthDataType) => {
  try {
    ...
    if (axios.isAxiosError(err)) {
      ...
      console.error('❌ Unexpected Error:', err);
      throw new Error('An unexpected error occurred while fetching the Market data.');
    }
  }
};
