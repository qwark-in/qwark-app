...
export const useFetchFipListMutation = () => {
  ...
  const { trigger, isMutating, data, error } = useSWRMutation<FetchFipListResponse, Error>(
    FIP_LIST_KEY,
    process.env.EXPO_PUBLIC_FEATURE_MOCK_API === 'true' ? mockFipListFetcher : fetcher,
    {
      onSuccess: ({ data }) => {
        setFips(
          data.fips
            .slice()
            .sort((a, b) => a.fip_name.toLowerCase().localeCompare(b.fip_name.toLowerCase()))
        );
        selectMultipleFips(
          data.fips
            .filter(
              (fip) =>
                fip.asset_class_id !== 'BANK' && selectedEntities.includes(fip.asset_class_id)
            )
            .map((fip) => ({
              fip_id: fip.fip_id,
  ...
  return {
    ...
    fetchFipListIsLoading: isMutating,
    fetchFipListData: data,
    fetchFipListError: error,
  };
};
