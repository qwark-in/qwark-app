 ...
 *
 * @description
 * Core hook for dispatching an HTTP/S request
 */
...
type CommonFetch = {
  ...
  queryBody?: { [index: string]: unknown } | null;
...
/**
 * Core logic for any HTTP/S request and its related state variables
 * @param params Props for the hook. Contains url, method and callback functions after the response.
 * @returns Handle to dispatch the request, and state variables for error, data,
 * HTTP status code and loading state.
 */
export function useFetch<TState>({ url, method }: UseFetchProps) {
  // <TState> turns this into a generic component. We will take advantage of this
  // by assigning the `data` variable the type TState.
  ...
  const commonFetch = async ({ queryParams, queryBody, onSuccess, onError }: CommonFetch) => {
    setIsLoading(true);
