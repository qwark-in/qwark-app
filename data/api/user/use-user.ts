...
import { axiosInstance } from '@/data/api/core/axios-provider';
...
export const useGetUser = (params: GetUserParams) => {
  ...
  const { data, isLoading, error, mutate } = useSWR<{ data: UserState }>(`/user`, _get, {
    onSuccess: ({ data }) => {
      setState(data);
    },
  });
...
export const useUpdateUser = (params: UpdateUserParams) => {
  const { commonFetch, isLoading, data, status, error } = useFetch<UpdateUserResponse>({
    url: `/user/${params.id}`,
    method: 'PATCH',
  });

  const updateUser = (queryParams: UpdateUserQueryParams, queryBody: UpdateUserQueryBody) => {
